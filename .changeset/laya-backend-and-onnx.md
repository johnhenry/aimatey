---
"@johnhenry/aimatey-native-onnx": minor
"@johnhenry/aimatey-native-laya": minor
"@johnhenry/aimatey-frontend": patch
"@johnhenry/aimatey-docs": patch
---

Add `LayaBackendAdapter`, running ConvAI's Laya typed-decision model in-process via `@receptron/laya` -- a real TypeScript/ONNX Runtime port (github.com/receptron/laya), not a hosted API. No network call, no API key, no Python service.
